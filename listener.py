...
ukr_videos_channel = config['output_channel_ids'][5]
ukr_photos_channel = config['output_channel_ids'][6]
...
@client.on(events.NewMessage(chats=ukr_input_channel_entities, func=lambda e: hasattr(e.media, 'document')))
async def handler(e):
    video = e.message.media.document
    if hasattr(video, 'mime_type') and bool(re.search('video', video.mime_type)):
        content = translator.translate(e.message.message)
...
@client.on(events.NewMessage(chats=ukr_input_channel_entities, func=lambda e: hasattr(e.media, 'photo')))
async def handler(e):
    chat = await e.get_chat()
    chat_name = get_chat_name(chat)
